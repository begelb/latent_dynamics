...
def _assert_finite_gradients(model: nn.Module, *, stage: str) -> None:
    missing: list[str] = []
    nonfinite: list[str] = []
    ...
    if missing or nonfinite:
        raise FloatingPointError(
            f"invalid {stage} gradients: missing={missing}, nonfinite={nonfinite}"
        )


def _assert_close(name: str, actual: float, expected: float, *, atol: float = 1e-12) -> None:
    if not math.isclose(actual, expected, rel_tol=0.0, abs_tol=atol):
        raise RuntimeError(
            f"source reconstruction mismatch for {name}: "
...
def _lbfgs_step(
    ...
    *,
    ...
    def closure() -> Tensor:
        ...
        total.backward()
        _assert_finite_gradients(model, stage="L-BFGS")
        return total
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    finite_arguments = {
        "adamw_lr_start": args.adamw_lr_start,
        "adamw_lr_end": args.adamw_lr_end,
        "adamw_weight_decay": args.adamw_weight_decay,
        "adamw_beta1": args.adamw_beta1,
        "adamw_beta2": args.adamw_beta2,
        "adamw_eps": args.adamw_eps,
        "adamw_gradient_clip": args.adamw_gradient_clip,
        "adamw_rho": args.adamw_rho,
        "lbfgs_lr": args.lbfgs_lr,
        "lbfgs_tolerance_grad": args.lbfgs_tolerance_grad,
        "lbfgs_tolerance_change": args.lbfgs_tolerance_change,
        "strict_relative_slack": args.strict_relative_slack,
        "strict_absolute_slack": args.strict_absolute_slack,
        "minimum_validation_improvement": args.minimum_validation_improvement,
    }
    nonfinite_arguments = [
        name for name, value in finite_arguments.items() if not math.isfinite(value)
    ]
    if nonfinite_arguments:
        raise ValueError(f"non-finite optimizer arguments: {nonfinite_arguments}")
    ...
    if args.adamw_weight_decay < 0 or args.adamw_rho <= 0:
        raise ValueError("AdamW weight decay must be nonnegative and rho positive")
    if args.adamw_eps <= 0 or args.adamw_gradient_clip <= 0:
        raise ValueError("AdamW epsilon and gradient clip must be positive")
    if not (0 <= args.adamw_beta1 < 1 and 0 <= args.adamw_beta2 < 1):
        raise ValueError("AdamW beta values must lie in [0, 1)")
    if any(not math.isfinite(rho) or rho <= 0 for rho in args.lbfgs_rho):
        raise ValueError("L-BFGS rho values must be positive")
    if args.lbfgs_outer_steps < 0:
        raise ValueError("L-BFGS outer steps must be nonnegative")
    if (
        args.lbfgs_lr <= 0
        or args.lbfgs_max_iter < 1
        or args.lbfgs_max_eval < 1
        or args.lbfgs_history_size < 1
        or args.lbfgs_tolerance_grad < 0
        or args.lbfgs_tolerance_change < 0
    ):
        raise ValueError("invalid L-BFGS optimizer arguments")
    if (
        args.strict_relative_slack < 0
        or args.strict_absolute_slack < 0
        or args.minimum_validation_improvement < 0
    ):
        raise ValueError("strict gate slacks and minimum improvement must be nonnegative")
    ...
    if _paths_overlap(output_dir, source_dir):
        raise ValueError("output directory must not overlap the accepted source directory")
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError(f"refusing to write nonempty output directory {output_dir}")
    output_dir.mkdir(parents=True, exist_ok=True)

    source_summary_path = _resolve(args.source_summary)
    source_summary_hash = _sha256(source_summary_path)
    if source_summary_hash != args.expected_source_summary_sha256:
        raise RuntimeError(
            "source summary hash mismatch: expected "
            f"{args.expected_source_summary_sha256}, got {source_summary_hash}"
        )
    source_summary = json.loads(source_summary_path.read_text())
    promoted_hashes = source_summary["promoted_checkpoint_sha256"]
    if promoted_hashes["autoencoder.pt"] != source_hash:
        raise RuntimeError("source summary does not register the accepted checkpoint hash")
    if promoted_hashes["autoencoder.json"] != _sha256(source_sidecar):
        raise RuntimeError("source summary does not register the architecture sidecar hash")
    if source_summary["experiment"] != cfg.experiment_name:
        raise RuntimeError("source summary experiment does not match the configuration")
    if int(source_summary["seed"]) != seed:
        raise RuntimeError("source summary seed does not match the configuration")
    if list(source_summary["configuration"]["training"]["loss_weights"]) != list(
        cfg.training.loss_weights
    ):
        raise RuntimeError("source summary loss weights do not match the configuration")
    ...
    warm_start = source_summary["warm_start"]
    if _sha256(relu_dir / "autoencoder.pt") != warm_start["autoencoder_sha256"]:
        raise RuntimeError("ReLU trust-reference checkpoint hash mismatch")
    if _sha256(relu_dir / "autoencoder.json") != warm_start["architecture_sidecar_sha256"]:
        raise RuntimeError("ReLU trust-reference architecture hash mismatch")
    ...
    expected_data_hashes = {
        "train_csv": source_summary["train_csv_sha256"],
        "validation_csv": source_summary["validation_csv_sha256"],
        "dataset_manifest": source_summary["dataset_manifest_sha256"],
        "train_metadata": source_summary["train_metadata_sha256"],
        "scaler": source_summary["scaler_sha256"],
    }
    mismatched_data = [
        name
        for name, expected_hash in expected_data_hashes.items()
        if data_provenance[name]["sha256"] != expected_hash
    ]
    if mismatched_data:
        raise RuntimeError(f"source data hash mismatch: {mismatched_data}")
    if data_provenance["train_rows"] != int(source_summary["full_batch_train_rows"]):
        raise RuntimeError("source training row count mismatch")
    if data_provenance["validation_rows"] != int(source_summary["validation_rows"]):
        raise RuntimeError("source validation row count mismatch")
    ...
    selected = source_summary["selected"]
    for component, expected in selected["replay"].items():
        _assert_close(
            f"selected.replay.{component}",
            baseline["validation"][component],
            float(expected),
        )
    for metric in (
        "max_anchor_normalized_l2",
        "characteristic_loss",
        "topology_loss",
        "max_characteristic_relative_error",
        "max_role_margin_violation",
        "trust_replay_mse",
        "trust_global_mse",
        "trust_global_rmse",
    ):
        _assert_close(metric, float(baseline[metric]), float(selected[metric]))
    strict_thresholds = {
    ...
    for step in range(1, args.adamw_steps + 1):
        ...
        objective.backward()
        _assert_finite_gradients(accepted_model, stage="AdamW")
        grad_norm = torch.nn.utils.clip_grad_norm_(
            accepted_model.latent_map.parameters(),
            args.adamw_gradient_clip,
    ...
    summary = {
        ...
        "source": {
            ...
            "summary_sha256": source_summary_hash,
            "input_contract_validated": True,
            "baseline_vector_reproduced_to_absolute_tolerance": 1e-12,
        },
        "config": args.config,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=EXPECTED_SOURCE_SUMMARY_SHA256,
    )
