...
| Fig (c) fine Morse sets + 4/5 inset zoom | replay | `scripts/plot_coarsened_morse.py --zoom-nodes 4 5` (via `render_paper_figures.py --only leslie3d_example1`) | fine morse_sets CSV | sets 4 and 5 shown at true size in a magnified inset, boxed and joined to their source; no inflation factor | `sec` |
...
| Fig `3D_Leslie_latent_coarse`(a) coarse (22,22,24) graph | fresh | `scripts/leslie3d_example1_uniform_grid.py --depth 22 --subdiv-max 24` | model + bounds; frozen `result.json` | 4 nodes: 0:(x^4−1,0,0), 1:(x^2−1,0,0), 2:(0,x^4−1,0), 3:(0,x+1,0); minimal {0,1}; node 3 (291 cells) holds fixed point + period-2 orbit; box widths (8.08e−4, 7.83e−4). Pinning max to 22 adds 20 one-cell components of trivial index that refinement resolves away; the four real sets are box-identical | 43 s + render |
| Fig (b) coarse (22,22,24) Morse sets | fresh | same + `scripts/plot_coarse_morse.py` | nontrivial CSV (89,327-box cover) | 4 nontrivial regions; bistability visible | `sec` |
...
| Fig (c,d) coarsened d=2 | fresh | `scripts/coarsen_chafee_infante.py` + `scripts/render_chafee_infante_standardized.py` | author weights + train data; quotient.json | Fine sets from the adaptive run (14,16,22); the merged fiber completed on the uniform grid at subdiv_min=16 (256×256, the basin grid). M(1) = 1,594 recurrent + 5,504 connection = 7,098 cells; quotient 3 nodes labelled 0,1,2. Completing on the adaptive graph instead gave 1,515 + 2,702 = 4,217 cells but put 7.4% of M(1) inside an attraction basin, against 0.18% here | `min` |
...
| Fig `ci_attractor_basins`(a,b) | replay | `scripts/plot_chafee_coarse_morse_roa.py` | author weights + data; `morse_roa_overlay.json` manifest | exactly 2 uniform attractors on 256×256 grid; 137 uniform Morse nodes; basin cells 15,941 / 15,764 | `min` |
