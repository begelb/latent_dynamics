r"""Reference full-batch training recipe for the Chafee--Infante studies.

The coauthor's archived paper computation uses one full batch and the
two-term objective
...
restoration, none of which occur in the archived Chafee--Infante loop.
...
REFERENCE_OBJECTIVE = "MSE(D(E(x)), x) + MSE(D(G(E(x))), y)"


@dataclass(frozen=True)
class ReferenceFullBatchResult:
...
def _seed_reference_run(seed: int) -> None:
...
def _full_batch_tensor(
    ...
    *,
    ...
    """Match the reference recipe's explicit float32 conversion and full-device transfer."""
...
def train_reference_full_batch(
    *,
    arch: ArchConfig,
    x: np.ndarray | Tensor,
    y: np.ndarray | Tensor,
    ...
    """Train with the exact optimization structure of the archived paper run.
    ...
    Args:
        ...
        verbose: Print progress every 100 epochs and at completion.

    Returns:
        A :class:`ReferenceFullBatchResult` containing the trained model, compact
    ...
    for epoch in range(epochs):
        ...
        # The reference loop monitors the same full-batch training objective,
        # after the optimizer update and without a validation metric.
    ...
    history_payload = {
        "schema_version": 1,
        "training_method": "reference_full_batch",
    ...
    return ReferenceFullBatchResult(
        model=model,
        history=history,
        summary=summary,
