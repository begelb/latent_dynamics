...
def test_runs_fixed_epochs_schedules_on_training_loss_and_saves_final_state(
    ...
    import latentdynamics.training.marcio as marcio_module

    arch = _tiny_arch()
    x, y = _tiny_pairs()
    seen_metrics: list[float] = []
    real_scheduler = marcio_module.ReduceLROnPlateau
    ...
    monkeypatch.setattr(marcio_module, "ReduceLROnPlateau", RecordingScheduler)
    result = train_marcio_full_batch(
        arch=arch,
        x=x,
        y=y,
...
def test_same_seed_reproduces_weights_history_and_compact_metadata(tmp_path):
    ...
    assert history_payload["training_method"] == "marcio_full_batch"
